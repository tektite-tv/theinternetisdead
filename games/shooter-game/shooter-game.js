/* ===========================================================
   Bananarama Apocalypse v5 — "INSANE BOSS MODE"
   ===========================================================
   ✦ Adds INSANE BOSS MODE: twin bosses at top-left and top-right
   ✦ Background upload & boss mode features intact
   =========================================================== */
...
bossBtn.onclick = () => startBossMode(false);
insaneBtn.onclick = () => startBossMode(true);

function startBossMode(insane = false) {
  menu.classList.add("hidden");
  document.getElementById("ui").classList.remove("hidden");
  resetGame();
  gameRunning = true;

  if (insane) spawnInsaneBossMode();
  else spawnBoss();
...
let bosses = []; // now supports multiple bosses
...
function spawnBoss(x = canvas.width/2, y = canvas.height/3) {
  const boss = {
    x, y, size:180, speed:1.2, img:bossImg, health:1000,
    orbiters:[], flashTimer:0, shootTimer:150
  };
  for (let i=0;i<4;i++){
    boss.orbiters.push({
