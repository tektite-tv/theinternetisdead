/* ===========================================================
   Bananarama Apocalypse v7 — "Health Resurgence"
   ===========================================================
   ✦ Progressive difficulty waves
   ✦ Twin bosses for INSANE mode
   ✦ Health bars restored for all bosses + orbiters
...
function draw() {
  ...
  bosses.forEach(boss => {
    ...
    ctx.fillRect(boss.x - bw / 2, boss.y - boss.size / 2 - 23, (boss.health / 1000) * bw, bh);
    ctx.strokeStyle = "#00ff99";
    ctx.strokeRect(boss.x - bw / 2 - 2, boss.y - boss.size / 2 - 25, bw + 4, bh + 4);

    // orbiters + mini health bars
    boss.orbiters.forEach(o => {
      ctx.drawImage(o.img, o.x - o.size / 2, o.y - o.size / 2, o.size, o.size);
      const ow = 80, oh = 8;
      ctx.fillStyle = "black";
      ctx.fillRect(o.x - ow / 2 - 1, o.y - o.size / 2 - 18, ow + 2, oh + 2);
      ctx.fillStyle = "#ff66cc";
      ctx.fillRect(o.x - ow / 2, o.y - o.size / 2 - 17, (o.health / 150) * ow, oh);
      ctx.strokeStyle = "#00ff99";
      ctx.strokeRect(o.x - ow / 2 - 1, o.y - o.size / 2 - 18, ow + 2, oh + 2);
    });
  });
...
function drawBar(label, x, y, value, max, color, alignBottom = false) {
  ...
  ctx.fillStyle = "black";
  ctx.fillRect(x - 2, baseY - 2, w + 4, h + 4);
  ctx.fillStyle = color;
  ctx.fillRect(x, baseY, (value / max) * w, h);
  ctx.strokeStyle = "#00ff99";
  ctx.strokeRect(x - 2, baseY - 2, w + 4, h + 4);
  ctx.fillStyle = "#00ff99";
