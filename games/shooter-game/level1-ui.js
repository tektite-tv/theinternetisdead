...
<style>
  ...
  #btnPauseQuit.pauseMenuBtn{ margin-top:16px; }
...
<div id="uiRoot">
  <div id="startMenu" class="panel">
    ...
    <div class="hint">Move: WASD or Arrow Keys (or Left Stick/D-Pad) · Aim: Mouse/Touch (or Right Stick) · Shoot: Space/Click (or RT/A) · Reach the glowing maze exit each wave · Pause: ESC (or Start) · Chat: / (or View)</div>
    <div class="btnRow btnRowTop">
      <button id="btnStart">Start Game</button>
      <button id="btnOptions">Options</button>
    </div>
    <div class="btnRow btnRowBottom">
      <button id="btnControls" class="smallBtn" type="button" aria-haspopup="true" aria-expanded="false" aria-live="polite" title="Keyboard / mouse active">Controls ⌨️</button>
    </div>
    <div class="controlsWrap">
      <div id="controlsMenu" class="panel" role="dialog" aria-label="Controls">
  ...
  <div id="optionsMenu" class="panel" style="display:none;">
    ...
    <div class="optionsGrid">
      <div class="optRow">
        <div class="optLabel"><span>Starting Lives</span><span id="livesVal">1</span></div>
        <input id="livesSlider" type="range" min="0" max="20" value="1" step="1" />
      </div>

      <div class="optRow">
        <div class="optLabel"><span>Starting Hearts</span><span id="heartsVal">1</span></div>
        <input id="heartsSlider" type="range" min="1" max="12" value="1" step="1" />
      ...
      <div class="optRow">
        ...
        <div class="hint">Shields act like extra one-hit armor pips (🛡️) next to your hearts.</div>
      </div>
      ...
      <div class="optRow checkboxGroup">
      ...
      <div class="optRow fullRow">
        ...
        <select id="startWaveSelect" style="width:100%; padding:10px 12px; font-family:monospace; font-size:14px; color:#00ff66; background:rgba(0,0,0,0.55); border:1px solid rgba(0,255,0,0.45); border-radius:12px; outline:none;">
          ...
          <option value="11">Bonus Mode</option>
        ...
        <div class="hint">Pick exactly where a new run should begin: Wave 1-10, Bonus Mode, or Insanity 1-10.</div>
      </div>
...
const AUTO_START_LEVEL = new URLSearchParams(window.location.search).get("autostart") === "1";
...
function showControlsPopup(){
  if (!controlsMenu || !btnControls || startMenu.style.display === "none") return;
  resetDraftBindingsFromActive();
  gameState = STATE.CONTROLS;
  controlsMenu.style.display = "block";
  btnControls.setAttribute("aria-expanded", "true");
  updateControlsDisplay();
  renderControlsBindingList();
  setControlsBindStatus('Select a control to rebind.');
  fitControlsMenuToViewport();
...
function hideControlsPopup(){
  if (!controlsMenu || !btnControls) return;
  resetDraftBindingsFromActive();
  controlsMenu.style.display = "none";
...
if (btnControls) btnControls.addEventListener("click", () => { if (controlsMenu && controlsMenu.classList.contains("visible")) hideControlsPopup(); else showControlsPopup(); });
