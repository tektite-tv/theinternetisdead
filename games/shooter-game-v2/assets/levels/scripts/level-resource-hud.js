function _syncStartResourceControls(){
  try{
    ...
    if (livesSlider) livesSlider.value = ((typeof START_LIVES_INFINITE !== "undefined" && START_LIVES_INFINITE) ? "100" : String(Math.max(0, parseInt(START_LIVES, 10) || 0)));
    if (heartsSlider) heartsSlider.value = ((typeof START_HEARTS_INFINITE !== "undefined" && START_HEARTS_INFINITE) ? "100" : String(Math.max(1, parseInt(START_HEARTS, 10) || 1)));
    if (shieldsSlider) shieldsSlider.value = ((typeof START_SHIELDS_INFINITE !== "undefined" && START_SHIELDS_INFINITE) ? "100" : String(Math.max(0, parseInt(START_SHIELDS, 10) || 0)));
    if (bombsSlider) bombsSlider.value = ((typeof START_BOMBS_INFINITE !== "undefined" && START_BOMBS_INFINITE) ? "100" : String(Math.max(0, parseInt(START_BOMBS, 10) || 0)));
