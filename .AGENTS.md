...
Use commit-message source tags:
- Any commit message written by Codex, ChatGPT, or another AI assistant must end with `[DEAD_COMMIT]`.
- Any commit message written manually by the user may end with `[ALIVE_COMMIT]`.
- When Codex commits and pushes repo edits, always append `[DEAD_COMMIT]` to the commit message unless the user explicitly overrides it.
